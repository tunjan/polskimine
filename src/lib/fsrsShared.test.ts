...
const DEFAULT_W = [
  0.40255, 1.18385, 3.173, 15.69105, 7.19605, 0.5345, 1.4604, 0.00405, 1.5491,
  0.10865, 1.1009, 2.7667, 0.08985, 0.32355, 1.36395, 0.0965, 2.7061,
];
...
const createLog = (
  overrides: Partial<ReviewLog> &
    Pick<
      ReviewLog,
      "state" | "elapsed_days" | "scheduled_days" | "grade" | "card_id"
    >,
): ReviewLog => ({
  id: `log-${++logCounter}`,
  stability: 5,
...
describe("fsrsShared", () => {
  ...
  describe("clampWeight", () => {
    ...
    it("should handle values just inside boundaries", () => {
      expect(clampWeight(WEIGHT_BOUNDS.min + 0.001)).toBe(
        WEIGHT_BOUNDS.min + 0.001,
      );
      expect(clampWeight(WEIGHT_BOUNDS.max - 0.001)).toBe(
        WEIGHT_BOUNDS.max - 0.001,
      );
    });
