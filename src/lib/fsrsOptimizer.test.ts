    ...
    setTimeout(() => {
      if (this.onmessage) {
        this.onmessage.call(
          ...
          { data: { type: "progress", progress: 50 } } as MessageEvent,
        );
        ...
        this.onmessage.call(
          this as unknown as Worker,
          { data: { type: "result", w: newW } } as MessageEvent,
        );
      }
    }, 10);
  }
...
describe("fsrsOptimizer", () => {
  ...
  it("should handle worker errors", async () => {
      ...
      postMessage(_data: any) {
        setTimeout(() => {
          if (this.onmessage) {
            this.onmessage.call(
              this as unknown as Worker,
              { data: { type: "error", error: "Test Error" } } as MessageEvent,
            );
          }
        }, 10);
      }
    ...
    await expect(optimizeFSRS(logs, currentW, onProgress)).rejects.toThrow(
      "Test Error",
    );
  });
});
