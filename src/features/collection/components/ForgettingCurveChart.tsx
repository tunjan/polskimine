...
function prepareData(revlog: ReviewLog[], targetEndDays: number, firstReviewTimestamp?: number) {
  ...
  const sortedLogs = [...revlog].sort((a, b) => a.created_at.localeCompare(b.created_at));
  
  
  const firstLogTime = sortedLogs.length > 0 ? new Date(sortedLogs[0].created_at).getTime() / 1000 : Date.now() / 1000;
  const effectiveFirstReviewTime = firstReviewTimestamp && firstReviewTimestamp < firstLogTime ? firstReviewTimestamp : firstLogTime;
  ...
  if (totalDaysToProject > 0) {
    const finalRetrievability = getRetrievability(totalDaysToProject, lastStability);
    data.push({
...
export const ForgettingCurveChart: React.FC<ForgettingCurveChartProps> = ({ 
  ...
  const firstReviewTimestamp = useMemo(() => {
    return firstReviewDate ? new Date(firstReviewDate).getTime() / 1000 : undefined;
  }, [firstReviewDate]);
  ...
  const targetEndDays = useMemo(() => {
    ...
    const sortedLogs = [...logs].sort((a, b) => a.created_at.localeCompare(b.created_at));
    ...
    const lastScheduledDays = sortedLogs[sortedLogs.length - 1].scheduled_days || 7;
  ...
  useEffect(() => {
    ...
    const yMin = Math.max(0, 100 - 1.2 * (100 - (d3.min(chartData, d => d.retrievability) || 0)));
    ...
    gradient.append("stop").attr("offset", `${desiredRetentionY}%`).attr("stop-color", "steelblue"); 
    ...
    svg.append("rect")
      ...
      .on("mousemove", (event) => {
        ...
        const index = d3.bisector((d: DataPoint) => d.date).left(chartData, date, 1);
        const d0 = chartData[index - 1];
        const d1 = chartData[index];
        const d = d1 && d0 ? (date.getTime() - d0.date.getTime() > d1.date.getTime() - date.getTime() ? d1 : d0) : (d0 || d1);
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <div className="flex justify-end gap-2">
        {(Object.keys(TimeRange) as (keyof typeof TimeRange)[]).filter(k => isNaN(Number(k))).map((key) => {
