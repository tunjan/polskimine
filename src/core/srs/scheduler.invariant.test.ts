...
const generateRandomCard = (seed: number): Card => {
  const statuses = [CardStatus.NEW, CardStatus.LEARNING, CardStatus.REVIEW, CardStatus.KNOWN];
  ...
  it("should maintain invariants across 1000 random card states", () => {
    ...
    for (let i = 0; i < 1000; i++) {
        ...
        try {
            ...
            if (result.status === CardStatus.LEARNING && result.state !== State.Relearning) {
            ...
            console.error("Fuzz failure on card:", JSON.stringify(card, null, 2), "Grade:", grade);
            throw e;
        }
    }
  });
